 ...
 */
private(TURN) struct bpf_cpumask __kptr *primary_cpumask;
 ...
 * CPU.
 */
s32 BPF_STRUCT_OPS(turn_select_cpu, struct task_struct *p, s32 prev_cpu, u64 wake_flags)
{
 ...
 */
void BPF_STRUCT_OPS(turn_enqueue, struct task_struct *p, u64 enq_flags)
{
 ...
 */
void BPF_STRUCT_OPS(turn_dispatch, s32 cpu, struct task_struct *prev)
{
...
void BPF_STRUCT_OPS(turn_running, struct task_struct *p)
{
	struct task_ctx *tctx;
	struct cpu_ctx *cctx;
...
void BPF_STRUCT_OPS(turn_stopping, struct task_struct *p, bool runnable)
{
	struct task_ctx *tctx;
	struct cpu_ctx *cctx;
...
void BPF_STRUCT_OPS(turn_quiescent, struct task_struct *p, u64 deq_flags)
{
	struct task_ctx *tctx;
...
void BPF_STRUCT_OPS(turn_enable, struct task_struct *p)
{
	scx_bpf_task_set_dsq_vtime(p, vtime_now);
}

void BPF_STRUCT_OPS(turn_disable, struct task_struct *p)
{
	/*
...
s32 BPF_STRUCT_OPS(turn_init_task, struct task_struct *p,
 ...
 */
void BPF_STRUCT_OPS(turn_exit, struct scx_exit_info *ei)
{
	UEI_RECORD(uei, ei);
}
 ...
 */
s32 BPF_STRUCT_OPS_SLEEPABLE(turn_init)
{
	s32 cpu;
...
SCX_OPS_DEFINE(turn_ops,
	       .select_cpu		= (void *)turn_select_cpu,
	       .enqueue			= (void *)turn_enqueue,
	       .dispatch		= (void *)turn_dispatch,
	       .running			= (void *)turn_running,
	       .stopping		= (void *)turn_stopping,
	       .quiescent		= (void *)turn_quiescent,
	       .enable			= (void *)turn_enable,
	       .disable			= (void *)turn_disable,
	       .init_task		= (void *)turn_init_task,
	       .init			= (void *)turn_init,
	       .exit			= (void *)turn_exit,
	       .timeout_ms		= 5000,
	       .name			= "turn");

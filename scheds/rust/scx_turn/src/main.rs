...
const SCHEDULER_NAME: &str = "scx_turn";
...
impl<'a> Scheduler<'a> {
    fn init(opts: &'a Opts, open_object: &'a mut MaybeUninit<OpenObject>) -> Result<Self> {
        ...
        let mut skel = scx_ops_open!(skel_builder, open_object, turn_ops, open_opts)?;

        skel.struct_ops.turn_ops_mut().exit_dump_len = opts.exit_dump_len;
        ...
        // Set scheduler flags.
        skel.struct_ops.turn_ops_mut().flags = *compat::SCX_OPS_ENQ_EXITING
        ...
        info!(
            "scheduler flags: {:#x}",
            skel.struct_ops.turn_ops_mut().flags
        );

        // Load the BPF program for validation.
        let mut skel = scx_ops_load!(skel, turn_ops, uei)?;
        ...
        let struct_ops = Some(scx_ops_attach!(skel, turn_ops)?);
